...
Datadog Pup CLI is packaged as `pkgs.pup` from official release archives and installed/exposed to OpenCode agents on `suremac` only (`pup`). The repo-managed `pup-cli` OpenCode skill is configured for `suremac` only; keep its public content concise and focused on low-token CLI patterns such as `--read-only`, `--no-agent`, `--jq`, bounded time windows/limits, and compact CSV/JSON output. Org-specific Datadog stack/ecosystem hints are secret because this repo is public: store them in `secrets/opencode-pup-stack-hints.age`, not in plaintext docs or skill files. `suremac` appends that decrypted secret to the local `pup-cli` skill during Home Manager activation.

The new Sentry CLI is packaged as `pkgs.sentry` from the npm `sentry` tarball and installed/exposed to OpenCode agents on `suremac` only (`sentry`). This is distinct from nixpkgs' legacy `sentry-cli`; the new CLI command is `sentry` and requires Node.js 22.15+. The wrapper disables Sentry's update check by default because updates are Nix-managed; do not use `sentry cli upgrade` for this install. The repo-managed `sentry-cli` OpenCode skill is configured for `suremac` only. Keep credentials out of Nix/plaintext docs: use `sentry auth login` and the CLI-managed `~/.sentry/` database, or add an agenix-backed module only if a concrete need for managed Sentry env defaults or token files appears. Update the package through `update-flakes --manual-packages-only --manual-package sentry --manual-version sentry=<version>` so the version and npm tarball hash change together.
