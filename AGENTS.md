...
Ctx is packaged from the SourceHut `ctx` flake input and exposed to OpenCode agents on `suremac` as `ctx` through `dotfiles.opencode.agentTools`, so primary agent runtime notes advertise local coding-agent history search. `suremac` enables `dotfiles.ctx.index`, a Home Manager launchd agent that runs `ctx setup --catalog-only --progress none` and `ctx import --all --resume --progress none` at load and every five minutes; logs live at `~/Library/Logs/ctx-index.log`. `tater` enables the same module via a systemd user timer. Update both host/root lock nodes when bumping the input.
