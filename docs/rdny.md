...
and can install rdny's upstream bundled OpenCode skills. The generated config
...
- `dotfiles.rdny.opencode.skill.enable` defaults to `true` and, when OpenCode is
  enabled, runs `rdny skills install --dir "$HOME/.config/opencode/skills"
  --force` during Home Manager activation. The bundled v0.3.0 skill is
  `rdny-browser`; content now comes from the selected rdny package instead of a
  dotfiles-managed copy.
...
exists. `suremac` therefore lists Google Chrome explicitly after Helium, while
the Nix-managed Helium path on `tater` is deterministic and needs no fallback.

## Host enablement

- `suremac` enables rdny, points `[binaries].chrome` at the manually installed
  Helium app with Google Chrome as an explicit fallback, enables ffmpeg for
  video assembly, configures a `helium` connect
