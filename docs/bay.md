...
- `bay sweep [repo]` removes configured build artifact paths from idle workspaces, including `.direnv`, `.devenv`, `result`/`result-*`, `target`, `node_modules`, and `.venv` by default. `bay sweep --all` reuses Bay's device discovery, deduplicates shared Jujutsu stores, and processes authoritative repository anchors sequentially; it reports a stable final count, warns and skips stale discovery candidates consistently with `bay list`, and continues after repository sweep failures. Repository failures are summarized as warnings after all healthy repositories are attempted. `--all` cannot be combined with a repository selector. Files and symlinks are removed without following symlinks. Both forms skip the current workspace, main checkout, and paths outside the canonical root. This removes local Nix root endpoints; store reclamation requires a later Nix GC.
