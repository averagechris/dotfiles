...
7. persist it with `review-artifact-write`
8. render it with `review-artifact-render`
9. use a question-chooser loop to triage candidate comments with the user
10. only post after explicit confirmation using `review-github-post`
...
After the digest and draft posting plan are ready, the workflow should use a
question-chooser loop to work through candidate comments with the user.
