...
enum Commands {
    ...
    /// Send content to Claude with metadata
    SendToClaudeAI {
        /// Source filename the content is from
        #[arg(long)]
        file: Option<String>,
        ...
        /// Content type (function, comment, selection, etc.)
        #[arg(long)]
        content_type: Option<String>,
        
        /// Flag if content is from saved file
        #[arg(long)]
        saved: bool,
        ...
        /// Force specific syntax highlighting language
        #[arg(long)]
        syntax: Option<String>,
        
        /// Skip metadata header completely
        #[arg(long)]
        no_metadata: bool,
        
        /// Mark as a code snippet rather than full file
        #[arg(long)]
        snippet: bool,
...
fn detect_file_type(filename: Option<&str>) -> Option<HashMap<String, String>> {
    ...
    file_info.insert("extension".to_string(), path.extension()
        .map(|e| e.to_string_lossy().to_string())
        .unwrap_or_default());
    file_info.insert("basename".to_string(), path.file_name()
        .map(|e| e.to_string_lossy().to_string())
        .unwrap_or_default());

    // Try to get mimetype
    if let Some(mime) = mime_guess::from_path(filename).first() {
        file_info.insert("mime_type".to_string(), mime.to_string());
    }
    ...
    if !ext.is_empty() {
        ...
        let (language, language_code) = match ext.as_str() {
            "py" => ("Python", "python"),
        ...
        if !language.is_empty() {
            file_info.insert("language".to_string(), language.to_string());
...
fn create_structured_message(
    ...
    let mut final_message = vec![
        ...
        "```".to_string(),
        "".to_string(),
        "The above JSON contains metadata about the following content.".to_string(),
    ];

    // Add custom header if provided
    if let Some(h) = header {
        final_message.push(h.to_string());
    }

    // Determine if we should add code block formatting
    let mut should_format_as_code = false;
    ...
    if let Some(f) = file {
        ...
        if let Some(s) = syntax {
            ...
            code_language = s.to_string();
        }
        // Otherwise try to detect from file info
        else if let Some(file_info) = detect_file_type(Some(f)) {
            if let Some(lang_code) = file_info.get("language_code") {
                if !lang_code.is_empty() {
                    should_format_as_code = true;
                    code_language = lang_code.clone();
                }
            }
        }
        // Fallback to common extension mapping
            ...
            "php", "sh", "nix", "html", "css", "json", "xml", "yaml", "md",
            "toml", "lua", "swift", "kt", "scala", "elm", "erl", "ex", "hs",
            "dart", "sql"
        ].contains(&ext.as_str()) {
            should_format_as_code = true;
            code_language = ext;
    ...
    final_message.push("".to_string());

    // Add content with proper formatting
    if should_format_as_code {
        final_message.push(format!("```{}", code_language));
        final_message.push(content.to_string());
        final_message.push("```".to_string());
    } else {
        final_message.push(content.to_string());
    }
...
/// Find a Claude window in WezTerm terminal
fn find_claude_window(wezterm: &WezTermClient) -> Result<Option<String>> {
    // Get all panes
    let panes = wezterm.get_all_panes()?;
    
    for pane in panes {
        // Check title for "claude" or "claude-code"
        let title = wezterm.get_pane_program(&pane)?;
...
/// Send message to Claude window or launch Claude directly
fn send_to_claude(
    wezterm_path: Option<PathBuf>,
    claude_path: Option<PathBuf>,
    message: &str,
    ...
    let claude = match claude_path {
        Some(path) => path,
        None => which("claude-code")
            .context("Claude CLI not found in PATH")?,
    };

    // Initialize WezTerm client if we're in WezTerm
    let wezterm = if let Ok(client) = WezTermClient::new(wezterm_path) {
        Some(client)
    } else {
        None
    };
    ...
    if let Some(wezterm_client) = wezterm {
        // Find Claude window
        if let Some(claude_pane) = find_claude_window(&wezterm_client)? {
            debug!("Found Claude pane: {}", claude_pane);
            
            // Send text to Claude window
            wezterm_client.send_text_to_pane(&claude_pane, message)?;
            
            // Send Enter key to submit
            wezterm_client.send_text_to_pane(&claude_pane, "\n")?;
            
            // Focus the window
            wezterm_client.activate_pane(&claude_pane)?;
            
            info!("Sent message to Claude window");
            return Ok(());
        }

        // No Claude window found - launch Claude directly
        debug!("No Claude window found, launching Claude CLI directly");
    }

    // Fall back to launching Claude CLI directly
    let status = Command::new(claude)
        .arg(message)
        .status()
        .context("Failed to launch Claude CLI")?;

    if !status.success() {
        return Err(anyhow!("Claude CLI failed with status: {}", status));
    }

    info!("Launched Claude CLI with message");
    Ok(())
}

/// Send content to Claude with metadata
fn send_to_claude_ai(
    wezterm_path: Option<PathBuf>,
    claude_path: Option<PathBuf>,
    file: Option<&str>,
    line: Option<&str>,
    ...
    let message = create_structured_message(
        file, line, column, content_type, saved, 
        header, syntax, no_metadata, snippet, &content
    );

    // Send to Claude
    send_to_claude(wezterm_path, claude_path, &message)
}

/// Add method to get all panes to the WezTermClient
impl WezTermClient {
    fn get_all_panes(&self) -> Result<Vec<String>> {
        // Get all panes in JSON format
        let output = Command::new(&self.wezterm_path)
            .args(["cli", "list", "--format=json"])
            .output()
        ...
        // Get all pane IDs
        let pane_ids: Vec<String> = panes.iter()
            .map(|pane| pane.pane_id.to_string())
            .collect();
...
fn main() -> Result<()> {
    ...
    match &cli.command {
        ...
        Commands::SendToClaudeAI { 
            file, line, column, content_type, saved, 
            header, syntax, no_metadata, snippet,
            wezterm_path, claude_path 
        } => send_to_claude_ai(
            wezterm_path.clone().or_else(|| cli.wezterm_path.clone()),
            claude_path.clone().or_else(|| cli.claude_path.clone()),
            file.as_deref(),
            line.as_deref(),
            column.as_deref(),
