...
Use the available Datadog skill, `pup` CLI, or integration. Inspect its instructions or help rather than assuming a command or MCP schema. Adapt to another available observability system such as New Relic, Honeycomb, Grafana, or Splunk. Start broad, then narrow. The operation names below are examples; map them to the available interface.
...
4. **Logs. Narrow, don't dump.**
   ...
   Redact credentials, tokens, cookies, personal data, and sensitive request or log payloads from excerpts. Keep the Datadog citation/identifier, timestamp, and useful non-sensitive fields, and mark omissions as `[REDACTED]`.
