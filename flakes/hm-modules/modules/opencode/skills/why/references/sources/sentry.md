...
Use the available Sentry skill, CLI, or integration. Inspect its instructions or help rather than assuming a command or MCP schema. Adapt to another available error tracker such as Rollbar, Bugsnag, or Airbrake. The operation names below are examples; map them to the available interface.
...
4. **Pull the full event for context.**
   ...
   Before quoting an event, redact credentials, tokens, cookies, personal data, and sensitive request or payload content. Preserve the issue/event citation, relevant frames, timestamps, and useful non-sensitive tags, marking omissions as `[REDACTED]`.
