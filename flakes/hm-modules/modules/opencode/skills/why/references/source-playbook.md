...
The why skill spawns one investigator per available evidence category, each reading a single source-specific playbook below. The playbooks are concrete examples for common tools and integrations; adapt them for a different interface in the same category.
