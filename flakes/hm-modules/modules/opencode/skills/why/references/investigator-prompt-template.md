...
- **Redact incidental sensitive data.** Remove credentials, tokens, cookies, personal data, and sensitive request or log payloads from quoted telemetry. Retain the source citation and enough non-sensitive context to establish relevance, marking omissions as `[REDACTED]`.
...
Use only the OpenCode tools, skills, CLIs, and integrations actually available. Inspect the applicable skill or command help rather than assuming a particular MCP or command schema. Never inspect `secrets/`, decrypted secret material, `.age` files, credentials, or authentication material. If a source is unavailable or unauthenticated, report the gap rather than trying to recover access.
...
3. **Follow links within your assigned source.** If a PR references another PR or commit, pull it. If a ticket links a parent or sibling, pull it. If a doc links another doc, pull it. Stay inside your assigned source. When you spot a cross-source reference, do NOT chase it yourself. Record it under "Additional Leads" for the parent to assess after the concurrent first wave and pursue in a bounded follow-up with the applicable source investigator or tool. Same-wave investigators cannot consume one another's output; chasing cross-source links also duplicates work and confuses scope.
