...
5. Prefer generating a draft artifact JSON payload with
   `review-artifact-generate` when the necessary PR metadata is available.
...
10. Apply `changes-review-core` to refine or fill gaps in the generated artifact
   when needed.
11. Persist the structured review artifact via `review-artifact-write`.
12. Build a compact terminal digest and draft posting plan from the artifact via
   `review-artifact-render`.
13. After evidence is stable and the digest is ready, use a question-chooser loop
    to work through candidate comments with the user.
14. Only if explicitly requested after that review loop, publish the review with
    `review-github-post`, passing an explicit final review state.
...
Before posting, use a question-chooser loop to work through the candidate
comments with the user. The purpose of the loop is comment triage, not workflow
debugging.

The chooser should only happen after:
...
The chooser can be used to decide, for each candidate finding or comment:

- keep as-is
- revise wording
- drop it
- convert between inline and top-level when appropriate
