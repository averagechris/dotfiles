{
  description = "Tom NixOS system configuration";
  ...
  outputs = inputs @ {
    ...
    mkHostWithModules = hostPath: let
      ...
      pkgs = import nixpkgs {
        ...
        config = {
          allowUnfree = true;
          # tom needs openssl-1.1.1w for home-assistant
  ...
  in {
    nixosConfigurations.tom = mkHostWithModules ./configuration.nix;

    deploy.nodes.tom = lib.mkDeploy' self.nixosConfigurations.tom;
  };
}
