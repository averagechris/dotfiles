# Empty overlay - titlecase is now handled directly in mkNixosHost/mkDarwinHost
{
  ...
  default = final: prev: {
    ...
    nix-openclaw = prev.nix-openclaw // {
      homeManagerModules = prev.nix-openclaw.homeManagerModules // {
        openclaw = { config, lib, pkgs, ... }: let
        ...
        in originalModule // {
          config = originalModule.config // {
            systemd = lib.mkIf pkgs.stdenv.hostPlatform.isLinux {
              user.services = lib.mkMerge (lib.mapAttrsToList (name: node: 
